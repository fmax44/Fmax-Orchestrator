import { formatRelayStatusText, RelayStatusService } from "../services/relayStatus.js";

const args = parseArgs(process.argv.slice(2));

if (!args.project) {
  console.error('Usage: npm run relay:status -- --project "D:\\projects\\some-project" [--format json] [--include-smoke] [--no-doctor] [--include-review] [--task 0001]');
  process.exitCode = 1;
} else {
  try {
    const result = await new RelayStatusService().check({
    ...
    if (args.format === "json") {
      ...
      console.log(formatRelayStatusText(result));
    }
